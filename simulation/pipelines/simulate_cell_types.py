...
#####################################################################################################
#' simulate_cell_types.py
#' This script describes the pipeline used to model how well the ISH scores are capable of detecting
#' differentially methylated regions between two cell types in a single sample.
#####################################################################################################
...
dmr_start = random.randint(1,len(result_list))
dmr_end = random.randint(dmr_start,len(result_list))
first_region = result_list[1:dmr_start]
dmr = result_list[dmr_start+1:dmr_end-1]
second_region = result_list[dmr_end:count]
...
out_file = open(genome_location+'/dmr/dmr.fa','wb')
out_file.write(dmr)
out_file.close()
...
dmr_meth = random.randint(0,100)

# Run Sherman to simulate reads
simulated = args.output_parent + "/" + args.sample_id + "/simulated/"
manager.run("mkdir " + simulated,simulated)
cmd = manager.config.tools.sherman + " --length " + str(manager.config.parameters.sherman.length) + " --number_of_seqs " + str(manager.config.parameters.sherman.number) + " --CG_conversion " + str(cell_meth) + " --CH_conversion " + str(manager.config.parameters.sherman.CH) + " --genome_folder " + args.output_parent + "/" + args.sample_id + "/first/ --outfolder " + simulated + " --outname cell_1_first " + " --error_rate " + str(manager.config.parameters.sherman.error_rate) + " --quality " + str(manager.config.parameters.sherman.quality) + " --rrbs"
...
# Run Sherman to simulate reads
simulated = args.output_parent + "/" + args.sample_id + "/simulated/"
cmd = manager.config.tools.sherman + " --length " + str(manager.config.parameters.sherman.length) + " --number_of_seqs " + str(manager.config.parameters.sherman.number) + " --CG_conversion " + str(cell_meth) + " --CH_conversion " + str(manager.config.parameters.sherman.CH) + " --genome_folder " + args.output_parent + "/" + args.sample_id + "/dmr/ --outfolder " + simulated + " --outname cell_1_dmr " + " --error_rate " + str(manager.config.parameters.sherman.error_rate) + " --quality " + str(manager.config.parameters.sherman.quality) + " --rrbs"
manager.run(cmd,lock_name=args.sample_id+'locker')

# Run Sherman to simulate reads
simulated = args.output_parent + "/" + args.sample_id + "/simulated/"
cmd = manager.config.tools.sherman + " --length " + str(manager.config.parameters.sherman.length) + " --number_of_seqs " + str(manager.config.parameters.sherman.number) + " --CG_conversion " + str(dmr_meth) + " --CH_conversion " + str(manager.config.parameters.sherman.CH) + " --genome_folder " + args.output_parent + "/" + args.sample_id + "/dmr/ --outfolder " + simulated + " --outname cell_2_dmr " + " --error_rate " + str(manager.config.parameters.sherman.error_rate) + " --quality " + str(manager.config.parameters.sherman.quality) + " --rrbs"
manager.run(cmd,lock_name=args.sample_id+'locker')

# Run Sherman to simulate reads
simulated = args.output_parent + "/" + args.sample_id + "/simulated/"
cmd = manager.config.tools.sherman + " --length " + str(manager.config.parameters.sherman.length) + " --number_of_seqs " + str(manager.config.parameters.sherman.number) + " --CG_conversion " + str(cell_meth) + " --CH_conversion " + str(manager.config.parameters.sherman.CH) + " --genome_folder " + args.output_parent + "/" + args.sample_id + "/second/ --outfolder " + simulated + " --outname cell_1_second " + " --error_rate " + str(manager.config.parameters.sherman.error_rate) + " --quality " + str(manager.config.parameters.sherman.quality) + " --rrbs"
manager.run(cmd,lock_name=args.sample_id+'locker')
...
# Map the simulated fastq file to the reference with bismark
