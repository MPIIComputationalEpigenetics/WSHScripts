...
parser.add_argument("--length",help="length of the simulated reads")
...
cmd = manager.config.tools.sherman + " --length " + str(args.length) + " --number_of_seqs " + str(args.number) + " --CG_conversion " + str(conversion_rate) + " --CH_conversion " + str(manager.config.parameters.sherman.CH) + " --genome_folder " + args.output_parent + "/" + args.sample_id + " --outfolder " + simulated + " --outname first " + " --error_rate " + str(manager.config.parameters.sherman.error_rate) + " --quality " + str(manager.config.parameters.sherman.quality) +" --rrbs"
...
#' Calculate epipolymorphism from methclone's output
...
#' Calculate methylation entropy from methclone's output
