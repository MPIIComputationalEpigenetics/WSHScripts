...
compute.qfdrp <- function(pos,cpg,reads,site){
	cpg <- cpg[[pos]]
	site <- site[[pos]]
...
#	rm(reads)
	...
	qfdrp <- sum(ret,na.rm=TRUE)/length(ret)
	qfdrp
}
...
calculate.qfdrps.chromosome <- function(bam, anno){
	...
	qfdrps_actual <- lapply(toApply,compute.qfdrp,match_cpg_reads,range_reads,starts_cpgs)
