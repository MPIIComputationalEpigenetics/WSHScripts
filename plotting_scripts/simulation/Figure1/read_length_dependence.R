...
agg <- aggregate(data,by=list(data$Read_Length),function(x){c(mean=mean(x,na.rm=TRUE),sd=sd(x,na.rm = TRUE))})
agg <- agg[,-c(2)]
melted <- c()
for(i in 2:7){
  frame <- agg[,i]
  add <- data.frame(Measure=colnames(data)[i],Read_Length=agg[,1],Mean=frame[,1],SD=frame[,2])
...
colors[1] <- "#00806fff"
colors[5] <- temp
plot <- ggplot(melted,aes(x=Read_Length,y=Mean,ymin=Mean-2*SD,ymax=Mean+2*SD,color=Measure,shape=Measure))+geom_point(size=3)+
