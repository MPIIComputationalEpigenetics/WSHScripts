...
#' error_dependence.R
#-----------------------------------------------------------------------------------------------------
#' This scripts produces a plot similar to Figure 1E to visualize the overall dependence of the scores
#' on sequencing errors. You should specify the folder, where the results of performing
#' the simulation experiment are stored as an argument and the folder where to store the results
#' as another.
...
data <- data.frame()
for(file in folders){
  error.level <- as.numeric(unlist(strsplit(file,"_"))[length(unlist(strsplit(file,"_")))])
