...
## ---------------------------------------------------------------------------------------------------------------------
## This script calculates the qFDRP values for all CpG positions in the given annotation with a
...
compute.discordant <- function(index,read1,read2,values,site){
	...
	# This is the only difference of qFDRP with respect to FDRP, which instead of classifying each read pair
	# now computes the fractions of sites that do not reflect the same methylation state in both reads
	length(intersection1)
}

#' compute.qfdrp
#' This function compute the qFDRP score for a given cpg site.
#'
...
#'					of the qFDRP, already converted into the custom representation
#' @param site:		position of the CpG site of interest as an integer number
#' @return:			qFDRP score for the given CpG site
#'
#' @details:	This function is called by calculate.qfdrps.chromosome each CpG present on
#' 				the chormosome and calls compute.discordant for each pair of reads.
#'
#' @author Michael Scherer

compute.qfdrp <- function(pos,cpg,reads,site,min.op){
	cpg <- cpg[[pos]]
	site <- site[[pos]]
...
#	rm(reads)
	...
	overlap <- findOverlaps(selected,selected,minoverlap=min.op,ignore.strand=TRUE)
	...
	#' now we start with the calculation of the qFDRP for the sample
	...
	ret <- unlist(ret)
	# This calculation does not change in the new version, since we are now interested in the average
	# fraction of sites that do not reflect the same methylation state on each read pair
	avg.overlap <- mean(ret,na.rm=TRUE)
	avg.overlap
}
...
#'
#' @details:	This function is called by calculate.qfdrps for each chromosome
#'				separately and calls toCpG as well as compute.qfdrp.
#'
#' @author Michael Scherer

calculate.qfdrps.chromosome <- function(bam, anno, min.op){
	...
	qfdrps <- as.list(rep(NA,length(anno)))
	rm(anno)
	...
	######### qFDRP CALCULATION ###########
	#' we only calculate the qFDRP for the CpGs that are acutally covered by any read in the
	#' corresponding sample
	logger.start(paste('qFDRP',chromosome))
	...
	qfdrps_actual <- lapply(toApply,compute.qfdrp,match_cpg_reads,range_reads,starts_cpgs,min.op)
	#' when we do not have a read that covers this site, we set the qFDRP for this site to NA
	qfdrps[null] <- qfdrps_actual
	qfdrps <- unlist(qfdrps)
...
#' calculate.qfdrps
#' This function calculates the qFDRPs for all CpG sites in
...
#' @param path:			path to a folder where the qFDRP CSV file should be written out
...
#'
#' This function writes the qFDRP scores into the given directory as a csv-file.
#'
#' @author Michael Scherer

calculate.qfdrps<- function(bam_file,anno,path,output_name,cores=1,window.size){
	...
	qfdrps <- foreach(chromosome=anno,.combine='c',.packages=c('RnBeads','GenomicAlignments','Rsamtools','rtracklayer'),.export=c('calculate.qfdrps.chromosome','compute.qfdrp','compute.discordant','toCpGs','bam','convert','restrict','WINDOW.SIZE')) %dopar%{
		calculate.qfdrps.chromosome(bam,chromosome)
	}
	stopCluster(cl)
	qfdrps <- unlist(qfdrps)
...
all.ops <- foreach(min.op=seq(10,100,by=5)) %dopar%{
	...
	for(fold in sel.files){
		bam.file <- BamFile(file.path(fold,"bam","sorted.bam"))
		if(!file.exists(file.path(fold,"rnbSet.zip"))) next
		rnb.set <- load.rnb.set(file.path(fold,"rnbSet.zip"))
		anno <- annotation(rnb.set)
		sel.chr <- count(anno$Chromosome)[which.max(count(anno$Chromosome)[,2]),1]
		anno <- anno[anno$Chromosome %in% sel.chr,]
